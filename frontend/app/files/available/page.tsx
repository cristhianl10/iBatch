...
export default function AvailableFilesPage() {
  ...
  const acceptUpload = async (file?: File) => {
    ...
    try {
      ...
      await refreshFiles();
      setSelectedId(uploaded.fileName);
      setNotice(`${uploaded.fileName} se cargo correctamente y esta listo para procesar.`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "No se pudo cargar el archivo.");
    } finally {
  ...
  return (
    <div className="application-shell">
      <header className="topbar">
        ...
        <button className="logout-button" type="button" onClick={async () => { await logout().catch(() => undefined); window.location.assign("/login"); }}>Salir</button>
      </header>
      ...
      <main>
        ...
        <section
          ...
          onDrop={(event) => { event.preventDefault(); void acceptUpload(event.dataTransfer.files[0]); }}
          aria-label="Cargar archivo CSV"
        >
          <div className="upload-zone__marker" aria-hidden="true">CSV</div>
          <div>
            <p className="eyebrow">Nuevo lote</p>
            <h2>{isUploading ? "Cargando y validando..." : "Arrastre aqui el archivo de transacciones"}</h2>
            <p>Se guardara en el directorio autorizado. Debe llamarse <strong>transactions_DDMMYYYY.csv</strong> e incluir cuenta, monto y fecha.</p>
          </div>
          <label className={`secondary-button upload-button ${isUploading ? "is-disabled" : ""}`}>
            Seleccionar CSV
            <input type="file" accept=".csv,text/csv" disabled={isUploading} onChange={(event) => void acceptUpload(event.target.files?.[0])} />
          </label>
        </section>
          ...
          <section className="live-progress-modal" role="dialog" aria-modal="true">
            ...
            <div className="progress-track-wrapper">
              ...
              <div className="progress-meta">
                ...
                <span>
                  <strong>{(progress.processedCount + progress.rejectedCount).toLocaleString("es-EC")}</strong> de <strong>{(progress.totalRecords > 0 ? progress.totalRecords : 10000).toLocaleString("es-EC")}</strong> filas
                </span>
              </div>
            </div>
