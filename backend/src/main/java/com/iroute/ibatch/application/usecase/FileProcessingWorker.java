...
public class FileProcessingWorker {
    ...
    public void process(Long fileId, InputFileMetadata inputFile) {
        try {
            fileProgressTracker.startProgress(fileId, inputFile.fileName());
            ...
            fileProgressTracker.finishProgress(
                    fileId,
                    inputFile.fileName(),
                    result.processedCount(),
                    result.rejectedCount(),
            ...
            processingLogRepository.save(fileId, null, LOG_ERROR, EVENT_PROCESS_ERROR, "El procesamiento del archivo fallo");
